...
## Measuring a PGDP corpus moved out

The five measurement subcommands left this repository on 2026-09-07. `rank-pgdp`, `profile-pgdp`,
`align-pgdp`, `typography-pgdp`, and `glyphs-pgdp` are now `pgdp-measure rank`, `profile`, `align`,
`typography`, and `glyphs`, in the `pdomain-pgdp-measure` package. Flags and arguments are
unchanged, and the reports carry the same wire contracts. Their walkthrough lives in that
package at `docs/usage/measuring-a-pgdp-corpus.md`.

This repository renders synthetic pages and publishes them. It consumes measurement reports; it no
