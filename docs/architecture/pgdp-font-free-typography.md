...
`typography-pgdp` reads a `pgdp-alignment/v3` report together with the `pgdp-profile/v2` profile
...
boundaries. See [the word-gap threshold finding](../research/2026-09-03-word-gap-threshold-is-too-low.md)
...
`typography-pgdp --geometry <book>.jsonl` adds an optional input: a JSONL file of OCR geometry
...
[F2 silently joins line-break hyphens](../research/2026-09-03-pgdp-f2-line-break-hyphens.md) for
the mechanism, and
...
`src/pdomain_ocr_synth/pgdp/ocr_witness.py` verifies each accepted page's `image_sha256` against
...
- Code: `src/pdomain_ocr_synth/pgdp/typography.py`,
  `src/pdomain_ocr_synth/pgdp/typography_measure.py`,
  `src/pdomain_ocr_synth/pgdp/typography_models.py`,
  `src/pdomain_ocr_synth/pgdp/typography_pooling.py`,
  `src/pdomain_ocr_synth/pgdp/ocr_witness.py`,
  `src/pdomain_ocr_synth/pgdp/alignment_image.py` (shared mask builder), and
