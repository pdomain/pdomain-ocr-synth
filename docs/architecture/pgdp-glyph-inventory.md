...
`glyphs-pgdp` cuts a labelled, per-character glyph inventory from one aligned book's own scans.
...
The command takes the same pinned inputs `typography-pgdp` does: a `pgdp-alignment/v3` report and
...
- Code: `src/pdomain_ocr_synth/pgdp/glyphs.py`, `glyph_cut.py`, `glyph_quality.py`,
  `glyph_shape.py`, `glyph_style.py`, `glyph_furniture.py`, `glyph_atlas.py`, `glyph_models.py`,
  and `src/pdomain_ocr_synth/cli.py`.
